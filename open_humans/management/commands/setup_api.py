...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        partners = [
            {
                'name': 'GoViral',
                'redirect_uri': (
                    'https://www.goviralstudy.com/' 'auth/open-humans/callback'
                ),
                'env_key': 'GO_VIRAL',
            },
            {
                'name': 'American Gut',
                'redirect_uri': (
                    'https://www.microbio.me/AmericanGut/' 'authed/connect/open-humans/'
                ),
                'env_key': 'AMERICAN_GUT',
            },
            {
                'name': 'Harvard Personal Genome Project',
                'redirect_uri': ('https://my.pgp-hms.org/' 'auth/open-humans/callback'),
                'env_key': 'PGP',
            },
            {
                'name': 'Wildlife of Our Homes',
                'redirect_uri': (
                    'http://wildlife.lvh.me:8007/' 'open_humans_complete/'
                ),
                'env_key': 'WILDLIFE',
            },
        ]
        ...
        for partner in partners:
            ...
            self.stdout.write(
                ('OPEN_HUMANS_CLIENT_ID="{}"'.format(application.client_id))
            )
            self.stdout.write(
                ('OPEN_HUMANS_CLIENT_SECRET="{}"'.format(application.client_secret))
            )
            self.stdout.write(
                ('OPEN_HUMANS_CALLBACK_URL="{}"'.format(application.redirect_uris))
            )
