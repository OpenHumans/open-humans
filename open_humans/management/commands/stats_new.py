...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for date in arrow.Arrow.range('day', start_cutoff, end_cutoff):
            ...
            data_connections = DataRequestProjectMember.objects.exclude(
                project__approved=False).exclude(joined=False).filter(
                member__user__is_active=True).exclude(authorized=False).exclude(
                project__returned_data_description='').filter(created__lte=cutoff)
